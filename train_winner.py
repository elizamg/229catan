...
from features.generate_winner_pred_features import build_winner_classification_dataset
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="logreg",
        choices=["logreg", "ridge", "xgb"],
        help="which model to train/evaluate (default: logreg; ridge kept as alias)",
    )
...
def main():
    ...
    model_name = "logreg" if args.model == "ridge" else args.model

    output_path = DEFAULT_WINNER_LOGREG_PATH if model_name == "logreg" else DEFAULT_WINNER_XGB_PATH
    ...
    # Random baselines on the same split
    rand_sampled = sampled_random_winner_accuracy(y_test, gid_test, seed=seed)

    # Save trained model.
    if not args.load:
        model_type = "winner_logreg" if model_name == "logreg" else "winner_xgb_classifier"
        joblib.dump(
            {
                "model": model,
                "scaler": scaler,
    ...
    print(
        ...
        f"(model={model_name}, n_test_games={len(np.unique(gid_test))})"
    )
