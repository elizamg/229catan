...
def make_model_path(args):
    return os.path.join(
        PROJECT_ROOT,
        f"winner_raw_crossattn_d{args.d_embed}_L{args.n_layers}_H{args.n_heads}"
        f"_ep{args.epochs}_lr{args.lr}_wd{args.weight_decay}.pt",
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train raw cross-attention winner model (no hand features)"
    )
    parser.add_argument(
...
def main():
    ...
    model = CatanWinnerRawCrossAttn(
        d_embed=args.d_embed, n_heads=args.n_heads, n_layers=args.n_layers,
    ).to(DEVICE)
