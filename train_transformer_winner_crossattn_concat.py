...
def make_model_path(args):
    return os.path.join(
        PROJECT_ROOT,
        f"winner_crossattn_concat_d{args.d_embed}_ep{args.epochs}_lr{args.lr}_wd{args.weight_decay}_do{args.dropout}.pt",
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train / evaluate Catan winner transformer (cross-attention + concat embeddings + hand features)"
    )
    parser.add_argument(
...
def main():
    ...
    model = CatanWinnerCrossAttnConcat(n_hand_feats_per_player, d_embed=args.d_embed, dropout=args.dropout).to(DEVICE)
