...
from transformer_winner import CatanWinnerTransformer
...
def split_by_game(data, winners, game_ids, test_frac=TEST_SPLIT, seed=RANDOM_SEED):
    """Split data by game ID. Each sample is one game, so this is a simple random split."""
...
def predict_all(model, loader):
    """Run model on all batches. Returns (logits, labels)."""
    model.eval()
    all_logits = []
    all_labels = []
    with torch.no_grad():
...
def evaluate(logits, winners):
    """Compute winner prediction accuracy."""
    preds = np.argmax(logits, axis=1)
    accuracy = (preds == winners).mean()
    return accuracy


DEFAULT_MODEL_PATH = os.path.join(PROJECT_ROOT, "winner_transformer_model.pt")


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train / evaluate Catan winner prediction transformer"
    )
    parser.add_argument(
        "--load",
        type=str,
...
def main():
    ...
    model = CatanWinnerTransformer(n_hand_feats_per_player).to(DEVICE)
