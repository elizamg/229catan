...
def make_model_path(args):
    return os.path.join(
        PROJECT_ROOT,
        f"winner_global_d{args.d_embed}_ep{args.epochs}_lr{args.lr}_wd{args.weight_decay}_do{args.dropout}.pt",
    )


def parse_args():
    parser = argparse.ArgumentParser(
        description="Train / evaluate Catan winner transformer (global pool + hand features)"
    )
    parser.add_argument(
...
def main():
    ...
    model = CatanWinnerTransformerGlobal(n_hand_feats_per_player, d_embed=args.d_embed, dropout=args.dropout).to(DEVICE)
