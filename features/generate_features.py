...
def extract_placement_features_winner(
    settlement_node: int,
    ...
    # Expansion potential: how good the *best legal* adjacent next settlement could be.
    # Consider settlement candidates one road step away from the road's far endpoint.
    # If occupied_nodes is provided, approximate the distance rule by requiring:
    # - candidate not occupied
    # - candidate not adjacent to any occupied node
    ...
    return features, distinct_resources
    ...
    for gid, players in game_players.items():
        ...
        for pi in range(len(players)):
            ...
            for rnd in (0, 1):
                ...
                placement_feats = extract_placement_features_winner(
                    settlement,
                    road,
                    tile_info,
                    port_map,
